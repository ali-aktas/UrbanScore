...
class BottomNavigationManager(
     ...
     *
     * @param backStackManager Back stack yönetimi için referans
     */
    fun setupBottomNavigation(backStackManager: BackStackManager) {
        ...
        bottomNavigation.setOnItemSelectedListener { item ->
            when (item.itemId) {
                in TAB_IDS -> {
                    handleTabSelection(item.itemId, backStackManager)
                    true
                }
                else -> {
        ...
        bottomNavigation.setOnItemReselectedListener { item ->
            // Tab yeniden seçildiğinde bir şey yapmıyoruz, varsayılan davranış
     ...
     * Tab seçimini işler
     *
     * @param tabId Seçilen tab ID'si
     * @param backStackManager Back stack yönetimi için referans
     */
    private fun handleTabSelection(tabId: Int, backStackManager: BackStackManager) {
        // Şu anki tab ile aynı mı kontrol et
        ...
        if (tabId != currentTabId) {
            ...
            Log.d(TAG, "Same tab selected: $tabId")
            // İsteğe bağlı: aynı tab seçildiğinde özel davranış tanımlanabilir
        }
    }
